...
serverPrefix=$(echo "${AZ_SERVER_PREFIX}" | tr '[:upper:]' '[:lower:]')

echo ".Please enter your new admin username:"
echo "     (Note - values should be lowercase and less than 8 characters.)" 
read -p "$(echo -e -n "${INPUT}.Admin Name:${RESET}")" JUMPBOX_SERVER_ADMIN_NAME
# This requires a newer version of BASH not avialble in MAC OS - AZ_SERVER_PREFIX=${AZ_SERVER_PREFIX,,} 
JUMPBOX_SERVER_ADMIN_NAME=$(echo "${JUMPBOX_SERVER_ADMIN_NAME}" | tr '[:upper:]' '[:lower:]')
...
do
  ...
  echo ""
  read -s -p "$(echo -e -n "${INPUT}.Re-enter to verify:${RESET}")" jumpboxPassword_confirm
  
  if [ $jumpboxPassword = $jumpboxPassword_confirm ]
  then
     break 2
  else
...
echo "      (Note - values should be lowercase and less than 8 characters.)"
read -p "$(echo -e -n "${INPUT}.Storage Prefix? (default: ${AZ_SERVER_PREFIX}demostorage):"${RESET})" AZ_STORAGE_PREFIX
[ -z "${AZ_STORAGE_PREFIX}" ] && AZ_STORAGE_PREFIX=${AZ_SERVER_PREFIX}
# This requires a newer version of BASH not avialble in MAC OS - AZ_STORAGE_PREFIX=${AZ_STORAGE_PREFIX,,} 
AZ_STORAGE_PREFIX=$(echo "${AZ_STORAGE_PREFIX}" | tr '[:upper:]' '[:lower:]')
...
if [[ $continuescript != "n" ]]; then
    ...
    sudo sed -i -e "s@VALUEOF-UNIQUE-SERVER-PREFIX@${AZ_SERVER_PREFIX}@g" ${SOURCEDIR}/environment/ossdemo-utility.json
...
if [[ $continuescript != "n" ]]; then
    ...
    if [ -f ~/.ssh/jumpbox_${AZ_SERVER_PREFIX}_id_rsa ]
    then
        echo "..Existing private key found.  Using this key ~/.ssh/jumpbox_${AZ_SERVER_PREFIX}_id_rsa for jumpbox creation"
    else
        echo "..Creating new key for ssh in ~/.ssh/jumpbox_${AZ_SERVER_PREFIX}_id_rsa"
        #Create key
        ssh-keygen -f ~/.ssh/jumpbox_${AZ_SERVER_PREFIX}_id_rsa -N "" -q
        #Add this key to the ssh config file 
    fi
    if grep -Fxq "Host jumpbox-${AZ_SERVER_PREFIX}.eastus.cloudapp.azure.com" ~/.ssh/config
    then
        # Replace the server with the right private key
        # BUG BUG - we need to actually replace the next three lines with new values
        # sed -i "s@*Host jumpbox-${AZ_SERVER_PREFIX}.eastus.cloudapp.azure.com*@Host=jumpbox-${AZ_SERVER_PREFIX}.eastus.cloudapp.azure.com IdentityFile=~/.ssh/jumpbox_${AZ_SERVER_PREFIX}_id_rsa User=${JUMPBOX_SERVER_ADMIN_NAME}@g" ~/.ssh/config
        echo "..We found an entry in ~/.ssh/config for this server - do not recreate."
    else
        # Add this to the config file
        echo -e "Host=jumpbox-${serverPrefix}.eastus.cloudapp.azure.com\nIdentityFile=~/.ssh/jumpbox_${serverPrefix}_id_rsa\nUser=${JUMPBOX_SERVER_ADMIN_NAME}" >> ~/.ssh/config
    fi
    ...
    --admin-username ${JUMPBOX_SERVER_ADMIN_NAME} \
...
sudo sed -i -e "s@VALUEOF_DEMO_ADMIN_USER@${JUMPBOX_SERVER_ADMIN_NAME}@g" ${SOURCEDIR}/ansible/jumpbox-server-configuration.yml
...
sudo sed -i -e "s@DEMO_ADMIN_USER=@DEMO_ADMIN_USER=${JUMPBOX_SERVER_ADMIN_NAME}@g" ${SOURCEDIR}/vm-assets/DemoEnvironmentValues

#Set the remote jumpbox passwords
echo "Resetting ${JUMPBOX_SERVER_ADMIN_NAME} and root passwords based on script values."
echo "Starting:"$(date)
ssh -t -o BatchMode=yes -o StrictHostKeyChecking=no ${JUMPBOX_SERVER_ADMIN_NAME}@jumpbox-${serverPrefix}.eastus.cloudapp.azure.com -i ~/.ssh/jumpbox_${serverPrefix}_id_rsa "echo '${JUMPBOX_SERVER_ADMIN_NAME}:${jumpboxPassword}' | sudo chpasswd"
ssh -t -o BatchMode=yes -o StrictHostKeyChecking=no ${JUMPBOX_SERVER_ADMIN_NAME}@jumpbox-${serverPrefix}.eastus.cloudapp.azure.com -i ~/.ssh/jumpbox_${serverPrefix}_id_rsa "echo 'root:${jumpboxPassword}' | sudo chpasswd"

#Copy the SSH private & public keys up to the jumpbox server
echo "Copying up the SSH Keys for demo purposes to the jumpbox ~/.ssh directories for ${JUMPBOX_SERVER_ADMIN_NAME} user."
echo "Starting:"$(date)
scp ~/.ssh/jumpbox_${serverPrefix}_id_rsa ${JUMPBOX_SERVER_ADMIN_NAME}@jumpbox-${serverPrefix}.eastus.cloudapp.azure.com:~/.ssh/id_rsa
scp ~/.ssh/jumpbox_${serverPrefix}_id_rsa.pub ${JUMPBOX_SERVER_ADMIN_NAME}@jumpbox-${serverPrefix}.eastus.cloudapp.azure.com:~/.ssh/id_rsa.pub
